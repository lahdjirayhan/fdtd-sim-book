...
def calculate_pml_parameters(npml, ie, je, ke):
    ...
    gk1, gk2, gk3,fk1, fk2, fk3

@numba.jit(nopython=True)
def calculate_dx_field(ie, je, ke, dx, idx, hy, hz,
gj3, gk3, gj2, gk2, gi1):
...
@numba.jit(nopython=True)
def calculate_dz_field(ie, je, ke, dz, idz, hx, hy,
gi3, gj3, gi2, gj2, gk1):
...
@numba.jit(nopython=True)
def calculate_e_fields(ie, je, ke, dx, dy, dz, gax, gay, gaz,
gbx, gby, gbz, ex, ey, ez, ix, iy, iz):
...
@numba.jit(nopython=True)
def calculate_fourier_transform_ex(ie, je, number_of_frequencies,
real_pt, imag_pt, ez, arg,
time_step, kc):
    ...
    for i in range(0, ie):
        for j in range(0, je):
            for m in range(0, number_of_frequencies):
                ...
                cos(arg[m] * time_step) * ez[i, j, kc]
                imag_pt[m, i, j] = imag_pt[m, i, j] - \
                sin(arg[m] * time_step) * ez[i, j, kc]
    return real_pt, imag_pt

@numba.jit(nopython=True)
def calculate_hx_field(ie, je, ke, hx, ihx, ey, ez,
fi1, fj2, fk2, fj3, fk3):
...
@numba.jit(nopython=True)
def calculate_hy_field(ie, je, ke, hy, ihy, ex, ez,
fj1, fi2, fk2, fi3, fk3):
...
@numba.jit(nopython=True)
def calculate_hz_field(ie, je, ke, hz, ihz, ex, ey,
fk1, fi2, fj2, fi3, fj3):
...
@numba.jit(nopython=True)
def calculate_hx_with_incident_field(ia, ib, ja, jb, ka, kb, hx,
ez_inc):
...
# Calculate the E field from the D field
ex, ey, ez, ix, iy, iz = calculate_e_fields(ie, je, ke, dx, dy, dz,
gax, gay, gaz,
gbx, gby, gbz,
ex, ey, ez, ix, iy, iz)
# Calculate the Fourier transform of Ex
...
hx, ihx = calculate_hx_field(ie, je, ke, hx, ihx, ey, ez,
fi1, fj2, fk2, fj3, fk3)
hx = calculate_hx_with_incident_field(ia, ib, ja, jb,
ka, kb, hx, ez_inc)
hy, ihy = calculate_hy_field(ie, je, ke, hy, ihy, ex, ez,
fj1, fi2, fk2, fi3, fk3)
hy = calculate_hy_with_incident_field(ia, ib, ja, jb,
ka, kb, hy, ez_inc)
hz, ihz = calculate_hz_field(ie, je, ke, hz, ihz, ex, ey,
fk1, fi2, fj2, fi3, fj3)
...
for m in range(number_of_frequencies):
    for j in range(ja, jb + 1):
        if gaz[ic, j, kc] < 1:
            amp[m, j] = 1 / (amp_in[m]) * sqrt(real_pt[m, ic, j, kc] ** 2 +
            imag_pt[m, ic, j, kc] ** 2)
